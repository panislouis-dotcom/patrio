...
**Correction found during independent verification (before Task 15 was dispatched, after Task 14 verification began):** `app/web/.gitignore` did NOT actually survive the `git reset --hard origin/main` this task performs — like the backend geometry layer (see Task 13's "Before Task 1" note), it apparently only existed on the old, fully-discarded branch and was never present on `origin/main`. This was only discovered when Task 14's verification ran `npm run build` and 89 stray `.js`/`.js.map` files appeared as *untracked* (not gitignored) in `git status --porcelain`, and a subsequent `vitest run` silently doubled its own report (36 test files / 200 tests instead of the real 18/100) by picking up the emitted `.test.js` siblings as independent test files. Rather than re-adding the `.gitignore` alone (which — as this very paragraph already says — only stops committing, not generating, and does nothing to stop vitest from collecting the stray files), the root cause was fixed directly: `app/web/tsconfig.json` had no `noEmit`, so plain `tsc` (as chained in `package.json`'s `"build": "tsc && vite build"`) emitted real `.js` output on every build even though `vite build` never reads it (vite transpiles independently via esbuild). Adding `"noEmit": true` to `tsconfig.json`'s `compilerOptions` stops the emission at the source — confirmed by deleting all stray files, running `npm run build` fresh, and observing zero new `.js`/`.js.map` siblings and a correct `vitest run` (18 files/100 tests). `app/web/.gitignore` was still added as specified above, as a second line of defense, but it is no longer the load-bearing fix. Both landed in their own commit (`28a47eb`, on top of Task 13's commits) since the gap was only surfaced during Task 14, not this task — Step 3 below (in the original Task 1, run before any of this existed) still applies as written for a *first* pass, but future re-reads of this plan should not assume the gitignore-only fix in this paragraph fully closes the footgun.
...
Note: this codebase has no `@testing-library/jest-dom` matcher setup (no `toBeInTheDocument`), so presence/absence assertions use plain `toBeTruthy()`/`toBeNull()` against `getByText`/`queryByText`, matching this file's established convention.
...
**Fixes applied post-implementation, after independent code-quality review (confirmed real, not implementer self-report):**
- **Stale-selection bug (confidence 90, confirmed):** `selectedFields`'s `<Section>` elements had no `key` tied to selection identity. Since `Field`'s `<input>` is uncontrolled (`defaultValue` only applied at mount), switching selection directly between two elements of the same kind (e.g. vertex A → vertex B, both dispatched via `SET_SEL` without an intervening `null`) reused the same DOM node and left the input showing A's stale value. Fixed by keying each `<Section>` branch to the selected element's identity (`sel.id` for vertex/edge, `` `${sel.edgeId}:${sel.index}` `` for openings), forcing a clean remount with a fresh `defaultValue` on every selection change.
- **`grossAreaM2` dead-code fallback (confirmed via reading `rooms.ts`'s `exteriorEdgeIds`/`traceFaces` directly):** the original `.find(f => f.edgeIds.every(...) && f.edgeIds.length === extEdges.size) ?? faces.reduce(...)` pattern re-derived the outer face by matching edge-id sets against `exteriorEdgeIds`'s output — but `exteriorEdgeIds` internally selects that same face via the identical "largest `Math.abs(area)`" reduce, so the `.find(...)` always succeeds and the `?? faces.reduce(...)` fallback was unreachable dead code. Simplified to directly reduce `traceFaces(floor)` by largest `Math.abs(area)` (dropping the now-unneeded `exteriorEdgeIds` import entirely) and dropped the redundant outer `Math.abs(shoelace(pts))` — `shoelace` (`geometry.ts:16`) already returns `Math.abs(shoelaceSigned(poly))`.
- **Test coverage gap:** added tests for `intWall_m`/`height_m`/`slab_m` field dispatches, edge selection (`SET_EDGE_THICKNESS`), opening selection for both door and window titles (`SET_OPENING_FIELD`), the empty-rooms message, and the `showDims` BIM-preview toggle (which defaults to `true` in `initialState`, not `false`).
...
**Root cause fixed during this task's execution:** Step 1 initially found 89 stray `.js`/`.js.map` files after `npm run build` — `app/web/.gitignore` (referenced above and in Task 1) turned out not to actually exist in this worktree (it was lost along with the rest of the old branch's state that Task 1 reset away), and even where it does exist, it only hides the files from `git status`, it does not stop `vitest run` from silently collecting the emitted `.test.js` siblings as duplicate test files (confirmed: this doubled the reported suite to 36 files/200 tests). Fixed at the root instead of re-applying the workaround every run: added `"noEmit": true` to `app/web/tsconfig.json`'s `compilerOptions` (nothing depends on `tsc`'s emitted `.js` — `vite build` transpiles independently via esbuild), plus re-added `app/web/.gitignore` as a secondary guard. Commit: `28a47eb`. With this fix, Steps 1/6 above should always find zero stray files with no manual deletion needed; the `find ... -delete` commands stay in the plan only as a defensive first pass in case some future tool invocation bypasses the project tsconfig (e.g. an explicit `tsc --outDir ...` elsewhere).
