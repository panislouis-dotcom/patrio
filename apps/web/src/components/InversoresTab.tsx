...
export function InversoresTab() {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true)
    try {
      const created = await createInvestor({ name: form.name.trim(), email: form.email.trim(), phone: form.phone.trim(), notes: form.notes.trim() })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 49px)' }}>
      ...
      {showCreate && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', borderBottom: `1px solid ${colors.border}`, flexWrap: 'wrap', background: colors.surfaceAlt }}>
          ...
          <button style={btnStyle('ghost')} onClick={() => { setShowCreate(false); setForm({ name: '', email: '', phone: '', notes: '' }) }}>
            CANCELAR
          </button>
        </div>
      )}
