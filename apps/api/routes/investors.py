...
def create_investor_route(body: InvestorCreate):
    return create_investor(body.model_dump())


@router.get("/api/investors/{investor_id}")
def get_investor_route(investor_id: int):
...
def update_investor_route(investor_id: int, body: InvestorUpdate):
    inv = get_investor(investor_id)
    if inv is None:
        raise HTTPException(status_code=404, detail="Investor not found")
...
def delete_investor_route(investor_id: int):
    inv = get_investor(investor_id)
    if inv is None:
        raise HTTPException(status_code=404, detail="Investor not found")
...
def list_project_investors(project_id: int):
    return get_project_investors(project_id)


@router.post("/api/projects/{project_id}/investors", status_code=201)
def upsert_project_investor_route(project_id: int, body: ProjectInvestorUpsert):
...
def update_project_investor_route(project_id: int, investor_id: int, body: ProjectInvestorUpsert):
...
def delete_project_investor_route(project_id: int, investor_id: int):
