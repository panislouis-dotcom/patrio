...
**Every line is an ordinary line.** A property born with the calculator gets exactly one, named with the arithmetic that produced it — «Estimado inicial · 200 m² × $9,000/m² × 1.3» — carrying the rough estimate from `sqmConstruction`, `constructionCostPerSqm` and the overhead. From that instant it is editable, renameable and deletable like any other, and **no metric ever rewrites it**: the calculator runs once, at birth, and there is no write path from the property's metrics into the budget afterwards — not automatic, not behind a button. Editing the m² or the $/m² of a property moves no money. The one thing that can replace it is an explicit whole-budget `apply` (below), which is the estimate becoming the breakdown that supersedes it.
...
**Applying never overwrites a line somebody captured by hand.** A source line whose `(chapter, name)` — lowercased and trimmed — already exists here is **skipped**, never updated: the one here may carry a supplier, a committed amount, payments or a close, and overwriting its price or quantity would rewrite money already captured. The one exception is the seeded estimate: a whole-budget `apply` onto a job that holds nothing else **deletes it** before copying, and that deletion is reported in neither `linesAdded` nor `linesSkipped`. So applying the same source twice adds nothing the second time, and `linesSkipped` says how much was left alone. Copying to several jobs is this same route called once per destination — there is no broadcast route, because each budget is independent and the correct atomicity is per property.
