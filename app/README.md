...
- **What the works will cost is the budget's sum, in every stage.** A property is born with a work budget — one line, «Otros, por detallar», carrying the rough estimate — and from that instant the sum of its lines *is* the construction cost that feeds `totalInvestment`. `m² × $/m² × overhead` is not a competing second source: it is the calculator that produces that first line and then retires, so no stage ever has two live answers to the same question. What was signed with a supplier and what left the bank are tracked against that plan as figures of their own, and never redefine it.
