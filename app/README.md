...
- **What the works will cost is the budget's sum, in every stage.** Always, with no second path: no target total, no line that absorbs the difference, no fallback for a property nobody has priced yet. A property is born with a work budget, and from that instant the sum of its lines *is* the construction cost that feeds `totalInvestment` — including when there are no lines and that sum is a real zero, which is a legitimate state («nothing has been costed yet») rather than a symptom. `m² × $/m² × overhead` runs **exactly once**, when the property is created, and lands as an ordinary line named after the arithmetic that produced it («Estimado inicial · 200 m² × $8,000/m² × 1.3», the overhead being 1.3 unless the property captured another — it appears in the name only when it actually multiplies, so a property with an overhead of 1 gets «Estimado inicial · 200 m² × $8,000/m²» and no trailing factor); from that moment it is edited, renamed and deleted like any other, and no metric ever rewrites it (an explicit whole-budget copy onto a property that holds nothing but that estimate does replace it — that is the estimate becoming its own breakdown). There is no write path from the property's metrics back into the budget — correcting the square metres afterwards moves the total by nothing, which is the whole point: a field labelled as a physical measurement used to reprice thirteen hand-quoted chapters. `constructionCostPerSqm` is therefore a captured assumption of yours again, shown next to the budget's own `budgetedCostPerSqm` (budget ÷ m²) — two independent numbers, neither one the other's fallback, which is the only arrangement in which comparing them says anything. What was signed with a supplier and what left the bank are tracked against that plan as figures of their own, and never redefine it.
