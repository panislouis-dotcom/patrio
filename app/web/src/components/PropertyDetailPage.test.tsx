...
const BASE_PROPERTY: Property = {
  ...
  projectedSale: 9_000_000, holdMonths: 12, rentMonthly: 30_000,
...
const RENTED: Property = {
  ...
  investmentBasis: 'manual', totalInvestment: 3_730_000,
...
async function renderPage(property: Property) {
  ...
  vi.mocked(api.clearPropertyFields).mockResolvedValue({ ...property, rentMonthly: null })
...
describe('PropertyDetailPage', () => {
  ...
  it('vaciar un campo pasa por clear-fields, con su propio botón', async () => {
    ...
    fireEvent.click(screen.getByText('EDITAR'))
    fireEvent.click(screen.getByLabelText('Vaciar RENTA/MES'))

    await waitFor(() => expect(api.clearPropertyFields).toHaveBeenCalledWith(7, ['rentMonthly']))
    expect(api.updateProperty).not.toHaveBeenCalled()
  })
})
