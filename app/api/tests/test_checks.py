...
def test_a_price_or_a_lot_of_zero_is_an_error():
    assert "landPrice" in _fields(_row(land_price=0), "error")
...
def test_soft_pre_purchase_warnings():
    ...
    assert "rentMonthly" in _fields(_row(rent_monthly=None), "warning")
...
def test_the_pre_purchase_warnings_stop_once_it_is_bought():
    ...
    fields = {i.field for i in _issues(_bought(rent_monthly=None, latitude=0))}
...
def test_desarrollo_warns_when_the_investment_was_typed_in():
    manual = _bought(land_price=None, total_investment=5_000_000)
    assert "totalInvestment" in _fields(manual, "warning")
    assert "totalInvestment" not in _fields(_bought(), "error")
...
def test_en_renta_without_a_captured_rent_is_an_error():
    ...
    row = _bought(status="en_renta", first_rent_date=date(2025, 6, 1), rent_monthly=None)
    assert "rentMonthly" in _fields(row, "error")


def test_vendida_wants_the_record_closed():
    sold = _row(status="vendida", sale_date=date(2026, 7, 1), sale_price=5_000_000,
                acquisition_date=None, land_price=None)
    fields = {i.field for i in _issues(sold)}
    assert "acquisitionDate" in fields   # no hold, so no realized ROI
    assert "totalInvestment" in fields   # no basis, so no realized gain
...
def test_a_zero_rent_is_never_admissible():
    assert "rentMonthly" in stage_requirements("prospecto", _row(rent_monthly=0))


def test_archivada_is_asked_for_nothing():
    assert stage_requirements("archivada", _row(land_price=None)) == {}
